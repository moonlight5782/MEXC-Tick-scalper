...
echo ============================================================
echo  TEMPORARY XRP_USDT - MEXC TESTNET EXECUTION TEST
...
.venv\Scripts\python.exe -m mexc_tick_scalper.auto_discovery_testnet_xrp ^
  --profit-runner-arm-bps 5 ^
  --target-closed-trades 100 ^
  --session-seconds 86400 ^
  --max-signals 3000 ^
  --testnet-position-poll-ms 250 ^
  --testnet-output persistent_end2end_TESTNET_XRP.csv 2>"%LOG_FILE%"

set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :failed

echo.
echo XRP Testnet runner finished normally. Exit code: %RC%
...
echo.
echo ============================================================
echo XRP TESTNET RUNNER FAILED. Exit code: %RC%
