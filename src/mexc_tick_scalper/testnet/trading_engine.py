...
class TestnetTradingEngine:
    ...
    async def _close_position(self, adapter: TestnetExecutionAdapter, position: ActivePosition, *, reason: str, exit_decision_ms: float | None = None) -> None:
    ...
    async def _manage_position(
        ...
        *,
        ...
        if mid_move <= -self.exit_policy.emergency_mid_adverse_bps:
            ...
            await self._close_position(adapter, position, reason="mid_adverse_cut", exit_decision_ms=decision_ms)
            return None
        ...
        try:
            ...
            decision_ms = time.time_ns() / 1_000_000.0
            self.console.print(f"EXIT DECISION {position.signal.symbol} reason=demo_price_unavailable -> close immediately")
            await self._close_position(adapter, position, reason="demo_price_unavailable", exit_decision_ms=decision_ms)
            return None

        executable_pnl_bps = signed_move_bps(position.signal.direction, position.entry_price, demo_exit_best)
        ...
        await self._close_position(adapter, position, reason=reason, exit_decision_ms=decision_ms)
        return None
